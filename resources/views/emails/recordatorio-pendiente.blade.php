...
<table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.15);">
  ...
  <tr><td style="background:{{ $tier === 2 ? '#b03a2e' : '#022858' }};padding:28px 32px;text-align:center;">
    <h1 style="color:#ffffff;font-size:22px;margin:0 0 4px;">
      {{ $tier === 1 ? 'Payment Reminder' : 'Urgent: Payment Reminder' }}
    </h1>
  ...
  <tr><td style="padding:24px 32px;">
    <p style="font-size:14px;color:#1a2a3a;margin:0 0 12px;">
      Your registration for <strong>{{ $registration->evento_nombre }}</strong> is still
      <strong style="color:#b07d00;">pending payment</strong>.
    </p>

    @if ($tier === 2)
      <p style="font-size:14px;color:#b03a2e;font-weight:700;margin:0 0 12px;background:#fdecea;border:1px solid #f5c6c1;border-radius:8px;padding:12px;">
        If payment is not received, the organizer will reassign your spot to another
        person {{ $diasGracia }} day{{ $diasGracia === 1 ? '' : 's' }} after this notice.
      </p>
    @endif
  ...
  <tr><td style="background:#022858;padding:20px 32px;text-align:center;">
    <p style="color:rgba(255,255,255,.6);font-size:11px;margin:0 0 2px;text-transform:uppercase;">Amount Due</p>
